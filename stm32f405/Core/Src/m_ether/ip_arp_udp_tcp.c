...
void init_ip_arp_udp_tcp(uint8_t *mymac, uint8_t *myip, uint16_t port) {
	...
	wwwport_l = (port & 0xff);
#ifdef _DEBUG_
	sprintf(debug_buf, "Port [%d%d]\r\n IP ", wwwport_h, wwwport_l);
	HAL_UART_Transmit(&DEBUG_UART, (uint8_t *)debug_buf, strlen((char *)debug_buf), 0xFFFF);
#endif
	while (i < 4) {
		ipaddr[i] = myip[i];
#ifdef _DEBUG_
		sprintf(debug_buf, "[%d]", ipaddr[i]);
		HAL_UART_Transmit(&DEBUG_UART, (uint8_t *)debug_buf, strlen((char *)debug_buf), 0xFFFF);
#endif
		i++;
	}
	i = 0;
#if 0
	sprintf(debug_buf, "\r\n MAC ", wwwport_h, wwwport_l);
	HAL_UART_Transmit(&DEBUG_UART, (uint8_t *)debug_buf, strlen((char *)debug_buf), 0xFFFF);
#endif
	while (i < 6) {
		macaddr[i] = mymac[i];
#ifdef _DEBUG_
		sprintf(debug_buf, "[%d]", macaddr[i]);
		HAL_UART_Transmit(&DEBUG_UART, (uint8_t *)debug_buf, strlen((char *)debug_buf), 0xFFFF);
#endif
		i++;
	}
...
#ifdef _DEBUG_
	sprintf(debug_buf, "GateWay ");
	HAL_UART_Transmit(&DEBUG_UART, (uint8_t *)debug_buf, strlen((char *)debug_buf), 0xFFFF);
#endif
	while (i < 4) {
		gwip[i] = gwipaddr[i];
#ifdef _DEBUG_
		sprintf(debug_buf, "[%d]", gwip[i]);
		HAL_UART_Transmit(&DEBUG_UART, (uint8_t *)debug_buf, strlen((char *)debug_buf), 0xFFFF);
#endif
		i++;
	}
...
//		Ethernet_PingCallback();
	...
	if (buf[TCP_DST_PORT_H_P] == TCPCLIENT_SRC_PORT_H)
	{
		...
		// @@@
#if 0
		sprintf(tx1_buff, "tcp: %d \r\n", buf);
		HAL_UART_Transmit(&huart1, (uint8_t *)tx1_buff, strlen(tx1_buff), 10);
#endif
