...
void Ethernet_make_tcp_ack_from_any(uint8_t *buf, int16_t datlentoack, uint8_t addflags);
...
// UDP - dirkx
void Ethernet_send_udp_data1(uint8_t *buf, uint16_t dlen, uint16_t source_port, uint8_t *dest_ip, uint16_t dest_port);
void Ethernet_send_udp_data2(uint8_t *buf, uint8_t *destmac, uint16_t dlen, uint16_t source_port, uint8_t *dest_ip, uint16_t dest_port);
...
uint16_t Ethernet_fill_tcp_data_len(uint8_t *buf, uint16_t pos, const char *s, uint16_t len);
// send data from the web server to the client:
void Ethernet_www_server_reply(uint8_t *buf, uint16_t dlen);
void Ethernet_wb_server_reply(uint8_t *buf, uint16_t dlen); // add by hans
// -- client functions --
...
void Ethernet_mk_net_str(char *resultstr, uint8_t *bytestr, uint16_t len, char separator, uint8_t base);
