...
uint16_t fill_tcp_data_len(uint8_t *buf, uint16_t pos, const char *s,
		uint16_t len);
...
void client_http_post(char *urlbuf, char *hoststr, char *additionalheaderline, char *method, char *postval,void (*callback)(uint8_t,uint16_t,uint16_t));
