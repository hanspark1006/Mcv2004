...
static char hostname[] = "Y&S System-00";
...
uint8_t dhcp_state(void) {
	...
	//
	if (dhcpState == DHCP_STATE_OK && (leaseStart + leaseTime) <= ENC28J60_GetMs()) {
...
void dhcp_send(uint8_t *buf, uint8_t requestType) {
	...
	send_udp_prepare(buf, (DHCPCLIENT_SRC_PORT_H << 8) | (dhcptid_l & 0xff),
			dhcpip, DHCP_DEST_PORT);
